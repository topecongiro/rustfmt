...
fn git_diff(commits: usize) -> String {
...
#[structopt(name = "git-rustfmt", about = "Format files in git commits", author = "")]
struct GitFmtOpts {
    /// Check only, don't format
    #[structopt(short = "c", long = "check")]
    check: bool,
    /// Format uncommitted files
    #[structopt(short = "u", long = "uncommitted")]
    uncommitted: bool,
...
fn main() {
    ...
    let opts = GitFmtOpts::from_args();

    if opts.check {
        eprintln!("error: `git-rustfmt --check` is not yet implemented");
        std::process::exit(1);
    }
    ...
    let stdout = git_diff(opts.commits);
