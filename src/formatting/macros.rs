...
use rustc_ast::tokenstream::{Cursor, TokenStream, TokenTree, LazyTokenStream, LazyTokenStreamInner};
...
pub(crate) fn convert_try_mac(
    ...
    if path == "try" || path == "r#try" {
        ...
        Some(ast::Expr {
            ...
            tokens: Some(LazyTokenStream::from(LazyTokenStreamInner::Ready(ts))),
        })
    } else {
        None
    }
